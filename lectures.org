...
Since \(\det \left(A_{\mathcal{E}\mathcal{F}} A_{\mathcal{F}\mathcal{G}}\right) = \det \left(A_{\mathcal{E}\mathcal{F}}\right) \det\left(A_{\mathcal{F}\mathcal{G}}\right)\), we do indeed have an equivalence class, and there are /precisely two equivalence classes/.

#+BEGIN_example
Compute the change of basis from \(\mathcal{E} = \{e_1, e_2\}\) to
\[
\{e_1, e_1 + e_2\}, \quad \{e_1, -e_2\}, \quad \{e_2, e_1\}.
\]
What corresponds to clockwise, and what corresponds to anti-clockwise?
#+END_example
